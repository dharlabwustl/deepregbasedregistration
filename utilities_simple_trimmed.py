...
def call_copy_affine(args):
    success=0
    try:
        moving_image=args.stuff[1]
        fixed_image=args.stuff[2]
        outputfilename=args.stuff[3]
        copy_affine(moving_image,fixed_image,outputfilename)
        ...
        pass
    return success
def copy_affine(moving_image,fixed_image,outputfilename):
    moving_image_data=nib.load(moving_image).get_fdata()
    fixed_image_nib=nib.load(fixed_image)
    region_img = nib.Nifti1Image(moving_image_data, affine=fixed_image_nib.affine, header=fixed_image_nib.header)
    nib.save(region_img, outputfilename)
...
#    for eachgrayfiles in allgrayfiles:
    if os.path.exists(betgrayfile):
        ...
        bet_nifti=nib.load(betgrayfile)
        bet_nifti_data=bet_nifti.dataobj.get_unscaled() #.get_fdata()
        gray_nifti_data[bet_nifti_data<np.max(bet_nifti_data)]= np.min(gray_nifti_data) #0 #
        array_img = nib.Nifti1Image(gray_nifti_data, affine=gray_nifti.affine, header=gray_nifti.header)
        nib.save(array_img, niifilenametosave)
        return niifilenametosave
    else:
...
def save_grayscale_slices_with_ventricles(gray_img_path, mask_img_path, out_file): ##output_dir="ventricle_slices"):
    """
    Extracts and saves grayscale 2D slices (NIfTI) where ventricle mask is non-zero in Z-plane.
    Preserves original header and affine.
    ...
    """
    # Load images
    gray_nii = nib.load(gray_img_path)
    mask_nii = nib.load(mask_img_path)

    gray_data = gray_nii.get_fdata()
...
def main():
    ...
    if name_of_the_function == "call_separate_mask_regions_into_individual_image":
        return_value=call_separate_mask_regions_into_individual_image(args) # call_copy_affine
    if name_of_the_function == "call_copy_affine":
        return_value=call_copy_affine(args)
