...
if [[ ${TYPE_OF_PROGRAM} ==   'APPLYDEEPREG_CSF_CISTERN_MIDLINE_SEP_COLIHM62' ]]; then
  ...
  /software/deepregbasedregis_csf_cistern_midline_separation_with_COLIHM62.sh $SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST /input /output
fi
...
if [[ ${TYPE_OF_PROGRAM} == 'APPLYDEEPREG_V2' ]]; then
  ...
  /software/deepregbasedregis_location_v2.sh $SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST /input /output
fi
if [[ ${TYPE_OF_PROGRAM} == 'APPLYDDFTOMASK' ]]; then
  echo " I AM AT TYPE_OF_PROGRAM==APPLYDDFTOMASK"
  /software/deepregbasedregis_location_v3.sh $SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST /input /output
fi
